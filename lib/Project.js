...
const schema = yup.object().shape({
	...
	servers: yup.array().of(yup.number().positive().integer()).required(0),
...
class Project {
	...
	async deploy() {
		...
		const git = await execa("git", [ "clone", `https://${github.username}:${github.password}@github.com/${project.repo}`, directory ]);
